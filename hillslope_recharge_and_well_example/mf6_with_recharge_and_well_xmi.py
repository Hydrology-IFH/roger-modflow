...
class ModFlowSimulation:
    def __init__(
        ...
        # self.recharge = np.zeros(self.n_active_cells)
        # self.head = np.empty(self.n_active_cells)
        # self.well_rate = np.zeros(self.n_active_cells)
        # self.max_iter = None

        h1 = np.min(bottom) + 10
        h2 = np.min(bottom) + 9
        h1_h2 = np.linspace(h1, h2, self.nrow)
        k = 0.1
        ...
        tdis = flopy.mf6.modflow.mftdis.ModflowTdis(
            sim, pname="tdis", time_units="DAYS", start_date_time="2020-01-01", nper=ndays, perioddata=[(1.0, 1, 1)] * ndays
        )
        ...
        for layer in range(0, nlay):
            for row in range(0, self.nrow):
                if row != 0 and row != self.nrow - 1:
                    # boundary to the west
                    chd_rec.append(((layer, row, 0), h1_h2[row]))
                    # boundary to the east
                    chd_rec.append(((layer, row, self.ncol - 1), h1_h2[row]))
            for col in range(0, self.ncol):
                # boundary to the north
                chd_rec.append(((layer, 0, col), h1))
                # boundary to the south
                chd_rec.append(((layer, self.nrow - 1, col), h2))
        ...
        well_locations = np.where(self.well_mask == True)  # only set wells where modflow_basin is True
        # 0: layer, 1: y-idx, 2: x-idx, 3: rate
        ...
        wells = flopy.mf6.ModflowGwfwel(gwf, print_input=False, print_flows=False, save_flows=False,
                                    maxbound=self.well_mask.sum(), stress_period_data=wells,
    ...
    def load_bmi(self):
        ...
        self.well_rate = self.mf6.get_value_ptr(well_tag)
    ...
    def set_recharge(self, recharge):
        ...
        recharge_tag = self.mf6.get_var_address("RECHARGE", self.name, "RCH_0")
        self.mf6.set_value(recharge_tag, recharge * (self.rowsize * self.colsize))

    def set_well_rate(self, well_rate):
        """Set recharge, value in m/day"""
        well_tag = self.mf6.get_var_address("BOUND", self.name, "WEL_0")
        self.mf6.set_value(well_tag, well_rate * (self.rowsize * self.colsize))
...
def main():
    ...
    well_mask = np.empty((NLAY, domain['nrow'], domain['ncol']))
    well_mask[:, :, :] = False
    well_mask[:, 6, 6] = True

    with rasterio.open(base_path / 'elevation.tiff', 'r') as src:
    ...
    for i in range(NDAYS):
        if i > 30:
            ...
            recharge = RNG.uniform(0.0, 0.0000002, size=domain['nrow'] * domain['ncol']).reshape(domain['nrow'], domain['ncol']).astype(np.float64)
            recharge[~mask] = np.nan
            mf.set_recharge(recharge)
            well_rate = -RNG.uniform(0.0, 0.002, size=domain['nrow'] * domain['ncol']).reshape(domain['nrow'], domain['ncol']).astype(np.float64)
            well_rate[~mask] = np.nan
            mf.set_recharge(well_rate)
