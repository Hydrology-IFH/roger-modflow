...
def main():
    ...
    for i in range(NDAYS):
        if i > 30:
            ...
            well_rate = -RNG.uniform(1.0, 20, size=domain['nrow'] * domain['ncol']).reshape(domain['nrow'], domain['ncol']).astype(np.float64)
            # well_rate[well_mask[0, :, :]] = -10
            well_rate[~well_mask[0, :, :]] = 0
