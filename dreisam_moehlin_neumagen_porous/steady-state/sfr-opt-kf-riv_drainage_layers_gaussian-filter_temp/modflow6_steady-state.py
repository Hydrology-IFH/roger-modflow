...
class ModFlowSimulation:
    def __init__(
        ...
        for ii in range(0, len(rows_bc)):
            ...
            ghb_rec.append(((layer, rows_bc[ii], cols_bc[ii]), constant_head, conductance, 10.0))
        ...
        recharge_temperature = np.full(recharge.shape, 10.0)  # set recharge temperature to 10 degree Celsius
        rcha = flopy.mf6.ModflowGwfrcha(gwf, recharge=recharge, fixed_cell=True, pname="rcha", auxiliary=["TEMP"], aux=recharge_temperature)
...
class MT3DSimulation:
    def __init__(
        ...
        gwt = flopy.mf6.ModflowGwt(self.gw_sim, modelname="gwt", save_flows=True)
        ...
        flopy.mf6.ModflowGwtic(gwt, strt=10)
        flopy.mf6.ModflowGwtadv(gwt, scheme="UPSTREAM")
        flopy.mf6.ModflowGwtdsp(gwt, alh=2.0, ath1=0.2)
        flopy.mf6.ModflowGwtmst(gwt, porosity=0.1)

        # SSM mapping: note the package name is RCHA-1 now
        ...
        flopy.mf6.ModflowGwtoc(
            gwt,
            concentration_filerecord="temp.ucn",
            budget_filerecord="gwt.cbc",
            saverecord=[("CONCENTRATION", "ALL"), ("BUDGET", "ALL")],
            printrecord=[("CONCENTRATION", "LAST")],
        )

        flopy.mf6.ModflowGwfgwt(self.gw_sim, exgtype="GWF6-GWT6", exgmnamea="gwf", exgmnameb="gwt")
        self.gw_sim.register_ims_package(self.ims, [self.gwf.name, gwt.name])
        # Create the MODFLOW 6 Input Files and Run the Model
        # Once all the flopy objects are created, it is very easy to create all of the input files and run the model.
        self.gw_sim.write_simulation()  # write the MODFLOW6 files
    ...
    def step(self):
        ...
        try:
            ...
            for i in range(self.max_iter):
                has_converged = self.mf6.solve(1)
                print(f"MT3D iteration {i+1} of {self.max_iter}. Convergence of numerical solution: {has_converged}")
            ...
            has_converged = False
            print("MT3D numerical solver timed out")
        finally:
            signal.alarm(0)  # Reset the alarm
            self.mf6.finalize_solve(1)
        ...
        if self.verbose:
            print(f"MT3D timestep {int(self.mf6.get_current_time())} converged in {round(time() - t0, 2)} seconds")
...
def main(model_run, well_id, converged):
    if converged == 1:
        ...
        mt3d_interface = MT3DSimulation(
            ...
            rowsize=modflow_config["dx"],
            colsize=modflow_config["dy"],
            gw_sim=modflow_interface.gw_sim,
            gw_ims=modflow_interface.ims,
            gwf=modflow_interface.gwf,
            verbose=True
        )

        # run MODPATH for one timestep
        converged = mt3d_interface.step()

        print("MT3D (steady-state) finalized")
