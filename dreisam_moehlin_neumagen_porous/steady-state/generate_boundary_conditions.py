...
def main(offset, plot):
    ...
    inner_boundary_black_forest = np.zeros((modflow_config['nx']+4, modflow_config['ny']+4))
    inner_boundary_black_forest[2:-2, 2:-2] = black_forest
    boundary_black_forest = np.zeros((modflow_config['nx']+4, modflow_config['ny']+4))
    boundary_black_forest[1:-3, 2:-2] = np.where(inner_boundary_black_forest[2:-2, 2:-2] - inner_boundary_black_forest[1:-3, 2:-2] < 0, 1, boundary_black_forest[1:-3, 2:-2])
    boundary_black_forest[3:-1, 2:-2] = np.where(inner_boundary_black_forest[2:-2, 2:-2] - inner_boundary_black_forest[3:-1, 2:-2] < 0, 1, boundary_black_forest[3:-1, 2:-2])
    boundary_black_forest[2:-2, 1:-3] = np.where(inner_boundary_black_forest[2:-2, 2:-2] - inner_boundary_black_forest[2:-2, 1:-3] < 0, 1, boundary_black_forest[2:-2, 1:-3])
    boundary_black_forest[2:-2, 3:-1] = np.where(inner_boundary_black_forest[2:-2, 2:-2] - inner_boundary_black_forest[2:-2, 3:-1] < 0, 1, boundary_black_forest[2:-2, 3:-1])
    boundary_black_forest[1:-3, 1:-3] = np.where(inner_boundary_black_forest[2:-2, 2:-2] - inner_boundary_black_forest[1:-3, 1:-3] < 0, 1, boundary_black_forest[1:-3, 1:-3])
    ...
    boundary_black_forest[51, :] = 0
    boundary_black_forest[:, 771] = 0
    boundary_black_forest[619, :] = 0
    cond = np.isfinite(topography)
    boundary_black_forest[cond == False] = 0

    mask_boundary_condition_fissured_aquifer = np.where((boundary_black_forest == 1), 1, np.nan)
    mask_boundary_condition_fissured_aquifer[mask_schoenberg == 1] = np.nan
    ...
    constant_head_fissured_aquifer = _topography - constant_head_fissured_aquifer
    constant_head_fissured_aquifer[constant_head_fissured_aquifer < 1] = _topography[constant_head_fissured_aquifer < 1] - 1
    ...
    with h5netcdf.File(params_file, "w", decode_vlen_strings=False) as f:
        ...
        v = f.create_variable(
            "mask_fissured_aquifer_bc", ("lat", "lon"), np.int32, compression="gzip", compression_opts=1
        )
        v[:, :] = mask_boundary_condition_fissured_aquifer[:, :]
        v.attrs.update(long_name="location of constant head boundary condition of the fissured aquifer", units="-")

        v = f.create_variable(
            "constant_head_fissured_aquifer", ("lat", "lon"), np.float32, compression="gzip", compression_opts=1
        )
        v[:, :] = constant_head_fissured_aquifer[:, :] - offset
        v.attrs.update(long_name="constant head boundary condition of the fissured aquifer", units="m a.s.l.")
        ...
        recharge = np.where(mask_black_forest == 1, np.nan, recharge)
