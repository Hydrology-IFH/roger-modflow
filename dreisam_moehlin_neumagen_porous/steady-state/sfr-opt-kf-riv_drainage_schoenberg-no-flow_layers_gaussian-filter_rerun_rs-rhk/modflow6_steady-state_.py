...
class ModFlowSimulation:
    def __init__(
        ...
        reaches.iloc[:, 9] = reaches.iloc[:, 9].astype(float) * 86400  # convert m/s to m/day
        ...
        reaches.iloc[:, 9] = reaches.iloc[:, 9].astype(float) / 86400
        rhk_std = (reaches.iloc[:, 9] - reaches.iloc[:, 9].mean()) / reaches.iloc[:, 9].std()
        rhk_new = fudge_parameters["rhk_scale"].values[model_run] * 10**(rhk_std/fudge_parameters["rhk_shape"].values[model_run]) * 86400
