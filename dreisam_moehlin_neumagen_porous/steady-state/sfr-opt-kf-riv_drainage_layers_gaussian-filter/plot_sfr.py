...
def main():
    ...
    axes.hist(reaches.iloc[:, 9], bins=np.logspace(np.log10(10e-10),np.log10(0.1), 90), color='blue', alpha=0.7)
    ...
    fig, axes = plt.subplots(figsize=(4, 4))
    axes.hist(reaches.iloc[:, 9] * 0.0001, bins=np.logspace(np.log10(10e-10),np.log10(0.1), 90), color='blue', alpha=0.7)
    # set log scale for x axis
    axes.set_xscale('log')
    axes.set_xlabel('Hydraulic Conductivity (m/s)')
    axes.set_ylabel('Frequency')
    file = base_path / "figures" / "dist_rhk_lower.png"
    fig.savefig(file, dpi=300)
    plt.close("all")

    fig, axes = plt.subplots(figsize=(4, 4))
    axes.hist(reaches.iloc[:, 9] * 0.05, bins=np.logspace(np.log10(10e-10),np.log10(0.1), 90), color='blue', alpha=0.7)
    # set log scale for x axis
    axes.set_xscale('log')
    axes.set_xlabel('Hydraulic Conductivity (m/s)')
    axes.set_ylabel('Frequency')
    file = base_path / "figures" / "dist_rhk_upper.png"
    fig.savefig(file, dpi=300)
    plt.close("all")
    ...
    axes.set_xlabel('Hydraulic Conductivity (m/s)')
    axes.set_ylabel('Frequency')
    file = base_path / "figures" / "dist_rhk_rs.png"
    fig.savefig(file, dpi=300)
    plt.close("all")
