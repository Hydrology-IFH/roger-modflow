...
def main(model_run):
    ...
    # recalculate rhk based on standardised streambed conductance distribution
    rhk_std = (reaches.iloc[:, 9] - reaches.iloc[:, 9].mean()) / reaches.iloc[:, 9].std()
    rhk_new = fudge_parameters["rhk_scale"].values[model_run] * 10**(rhk_std/fudge_parameters["rhk_shape"].values[model_run])
    reaches.loc[:, "rhk"] = rhk_new
    ...
    attrs = dict(
            ...
            title="MODFLOW steady-state simulation of the water protection area Zartener Becken",
    ...
    data_vars=dict(
            mask=(["lat", "lon"], mask_wsg_zarten[y1:y2, x1:x2]),
            gw_head=(["Time", "layer", "lat", "lon"], ds_mf['head'].values[:, :, y1:y2, x1:x2]),
            gw_depth=(["Time", "layer", "lat", "lon"], gw_depth[np.newaxis, :, y1:y2, x1:x2]),
            indirect_recharge=(["Time", "lat", "lon"], indirect_recharge_zarten[np.newaxis, :, :] * (-1)),
            direct_recharge=(["Time", "lat", "lon"], direct_recharge_zarten[np.newaxis, :, :]),
            ...
            kf_fudged=(["layer", "lat", "lon"], kf_fudged[:, y1:y2, x1:x2]),
            ...
            gw_extraction=(["Time", "layer", "lat", "lon"], gw_extraction[np.newaxis, :, y1:y2, x1:x2]),

        )

    ds = xr.Dataset(data_vars=data_vars, coords=coords, attrs=attrs)
    ds["mask"].attrs["long_name"] = "Mask of the water protection area Zartener Becken"
    ...
    attrs = dict(
            date_created=datetime.datetime.today().isoformat(),
            title="MODFLOW steady-state simulation of the water protection area Hausen",
            institution="University of Freiburg, Chair of Hydrology",
            flopy_version=f"{flopy.__version__}",
            modflow_version=f"{ml.version}",
        )
    ...
    data_vars=dict(
            mask=(["lat", "lon"], mask_wsg_hausen[y1:y2, x1:x2]),
            gw_head=(["Time", "layer", "lat", "lon"], ds_mf['head'].values[:, :, y1:y2, x1:x2]),
            gw_depth=(["Time", "layer", "lat", "lon"], gw_depth[np.newaxis, :, y1:y2, x1:x2]),
            indirect_recharge=(["Time", "lat", "lon"], indirect_recharge_hausen[np.newaxis, :, :] * (-1)),
            direct_recharge=(["Time", "lat", "lon"], direct_recharge_hausen[np.newaxis, :, :]),
            elevations=(["z", "lat", "lon"], ds_params['elevations'].values[:, y1:y2, x1:x2]),
            thickness=(["layer", "lat", "lon"], thickness[:, y1:y2, x1:x2]),
            kf=(["layer", "lat", "lon"], ds_params["kf"].values[:, y1:y2, x1:x2] / 86400),
            sy=(["layer", "lat", "lon"], sy[:, y1:y2, x1:x2]),
    ...
    ds = xr.Dataset(data_vars=data_vars, coords=coords, attrs=attrs)
    ds["mask"].attrs["long_name"] = "Mask of the water protection area Hausen"
    ds["indirect_recharge"].attrs["units"] = "m3/day"
    ds["indirect_recharge"].attrs["long_name"] = "Recharge from surface water. Negative values indicate surface water leakage into the groundwater."
    ds["direct_recharge"].attrs["units"] = "m3/day"
    ds["direct_recharge"].attrs["long_name"] = "Recharge from percolation"
