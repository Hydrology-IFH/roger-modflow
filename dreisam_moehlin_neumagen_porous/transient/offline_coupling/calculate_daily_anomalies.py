...
def main(model_run, area):
    ...
    for stress_test_scenario in stress_test_scenarios:
        ...
        # save the metrics to csv
        output_file = base_path / "output" / f"daily_values_run{model_run}_{area}.csv"
        df_metrics.to_csv(output_file, index=False, sep=";")
        output_file = base_path / "output" / f"daily_anomalies_abs_run{model_run}_{area}.csv"
        df_anomaly_metrics_abs.to_csv(output_file, index=False, sep=";")
        output_file = base_path / "output" / f"daily_anomalies_rel_run{model_run}_{area}.csv"
