...
def main(model_run):
    ...
    _units_abs = {"air_temperature": " [°C]",
                  ...
                  "direct_recharge": " [mm/Jahr]", 
                  "indirect_recharge": " [Mio. $m^3$/Jahr]", 
                  "gw_depth": " [m]",
                  "well_extraction": " [Mio. $m^3$/Jahr]",
                  "irrigation": " [mm/Jahr]"}
    ...
    for area in areas:
        for stress_test_scenario in stress_test_scenarios:
            for period in periods:
                ...
                fig, ax = plt.subplots(nrows=1, ncols=len(variables), figsize=(len(variables)*1.5, 2))
                for i, variable in enumerate(variables):
                    ...
                    if variable in ["direct_recharge", "indirect_recharge", "well_extraction"]:
                        vals = [v / 1e6 for v in vals]  # convert to million m3/year
                    ax[i].boxplot(vals, positions=[0], patch_artist=True, boxprops=dict(facecolor="grey"), whis=[5, 95])
                    # remove x-ticks
                    ax[i].set_xticks([])
                    ax[i].set_xlabel("")
            ...
            if stress_test_scenario != "base-magnitude0-duration0_no-irrigation_no-yellow-mustard_soil-compaction":
                for period in periods:
                    ...
                    for metric in metrics:
                        ...
                        for i, variable in enumerate(variables):
                            ...
                            if variable in ["direct_recharge", "indirect_recharge", "well_extraction"]:
                                val = df["value"].values[0] / 1e6  # convert to million m3/year
                            else:
