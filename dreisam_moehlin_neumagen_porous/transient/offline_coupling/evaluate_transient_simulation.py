...
def main(model_run):
    ...
    fig, axes = plt.subplots(figsize=(4, 4))
    plt.scatter(wells_obs_x, wells_obs_y, c=metric_values, s=5, cmap=cm, vmin=0, vmax=5)
    ...
    for gauge in streamflow_gauges:
        ...
        axes.set_ylim(0,)
        axes.set_title(f"MAE: {df_metrics_sfr.loc[gauge, 'MAE']:.2f} m, r: {df_metrics_sfr.loc[gauge, 'r']:.2f}")
        axes.set_xlabel("Zeit")
