...
def main():
    ...
    for scenario_flag, script_name in zip(scenario_flags, script_names_modflow):
        ...
        # lines.append("# Remove files from workspace\n")
        ...
        lines.append('mv -v "${TMPDIR}"/roger-modflow/dreisam_moehlin_neumagen_porous/transient/offline_coupling/output/%s/{*,.[!.]*} %s\n' % (script_name, output_path_ws.as_posix()))
        lines.append("cd ${TMPDIR}/roger-modflow/dreisam_moehlin_neumagen_porous/transient/offline_coupling/output\n")
        lines.append('ls\n')
        lines.append('cd %s\n' % (script_name))
        lines.append('ls\n')
