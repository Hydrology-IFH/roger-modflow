...
def main():
    ...
    for stress_test_meteo in stress_tests_meteo:
        if stress_test_meteo == "base":
            ...
            script_names.append('write_modflow_data_%s_soil-compaction' % (stress_test_meteo))
            script_names.append('write_modflow_data_%s' % (stress_test_meteo))
            script_names.append('write_modflow_data_%s_soil-compaction_irrigation' % (stress_test_meteo))
            script_names.append('write_modflow_data_%s_soil-compaction_yellow-mustard' % (stress_test_meteo))
            script_names.append('write_modflow_data_%s_irrigation_yellow-mustard' % (stress_test_meteo))
            script_names.append('write_modflow_data_%s_soil-compaction_irrigation_yellow-mustard' % (stress_test_meteo))
            ...
            for magnitude in stress_test_meteo_magnitudes:
                for duration in stress_test_meteo_durations:
                    if magnitude == 0 and duration == 0:
                        ...
                        script_names.append('write_modflow_data_%s_magnitude%s_duration%s_soil-compaction' % (stress_test_meteo, magnitude, duration))
                        script_names.append('write_modflow_data_%s_magnitude%s_duration%s' % (stress_test_meteo, magnitude, duration))
                        script_names.append('write_modflow_data_%s_magnitude%s_duration%s_soil-compaction_irrigation' % (stress_test_meteo, magnitude, duration))
                        script_names.append('write_modflow_data_%s_magnitude%s_duration%s_soil-compaction_yellow-mustard' % (stress_test_meteo, magnitude, duration))
                        ...
                        if stress_test_meteo in ["spring-summer-drought", "summer-drought"] and magnitude == 2 and duration == 3:
                            ...
                            script_names.append('write_modflow_data_%s-magnitude%s-duration%s_soil-compaction_well-extraction-stress' % (stress_test_meteo, magnitude, duration))
                            script_names.append('write_modflow_data_%s-magnitude%s-duration%s_irrigation_soil-compaction_well-extraction-stress' % (stress_test_meteo, magnitude, duration))
