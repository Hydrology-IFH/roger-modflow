...
def main():
    ...
    for stress_test_meteo in stress_tests_meteo:
        if stress_test_meteo == "base":
            ...
            script_names.append('write_modflow_data_%s-magnitude0-duration0_no-irrigation_no-yellow-mustard_soil-compaction' % (stress_test_meteo))
            script_names.append('write_modflow_data_%s-magnitude0-duration0_irrigation_no-yellow-mustard_soil-compaction' % (stress_test_meteo))
            script_names.append('write_modflow_data_%s-magnitude0-duration0_no-irrigation_yellow-mustard_no-soil-compaction' % (stress_test_meteo))
            script_names.append('write_modflow_data_%s-magnitude0-duration0_irrigation_yellow-mustard_no-soil-compaction' % (stress_test_meteo))
            script_names.append('write_modflow_data_%s-magnitude0-duration0_no-irrigation_no-yellow-mustard_soil-compaction_grain-corn-only' % (stress_test_meteo))
            ...
            for magnitude in stress_test_meteo_magnitudes:
                for duration in stress_test_meteo_durations:
                    if magnitude == 0 and duration == 0:
                        ...
                        script_names.append('write_modflow_data_%s-magnitude%s-duration%s_no-irrigation_no-yellow-mustard_soil-compaction' % (stress_test_meteo, magnitude, duration))
                        script_names.append('write_modflow_data_%s-magnitude%s-duration%s_irrigation_no-yellow-mustard_soil-compaction' % (stress_test_meteo, magnitude, duration))
                        script_names.append('write_modflow_data_%s-magnitude%s-duration%s_no-irrigation_yellow-mustard_no-soil-compaction' % (stress_test_meteo, magnitude, duration))                        
                        script_names.append('write_modflow_data_%s-magnitude%s-duration%s_irrigation_yellow-mustard_no-soil-compaction' % (stress_test_meteo, magnitude, duration))
                        script_names.append('write_modflow_data_%s-magnitude%s-duration%s_no-irrigation_no-yellow-mustard_soil-compaction_grain-corn-only' % (stress_test_meteo, magnitude, duration))

                        if stress_test_meteo in ["summer-drought"] and magnitude == 2 and duration == 3:
                            ...
                            script_names.append('write_modflow_data_%s-magnitude%s-duration%s_no-irrigation_no-yellow-mustard_soil-compaction_well-extraction-stress' % (stress_test_meteo, magnitude, duration))
                            script_names.append('write_modflow_data_%s-magnitude%s-duration%s_irrigation_no-yellow-mustard_soil-compaction_well-extraction-stress' % (stress_test_meteo, magnitude, duration))

                        elif stress_test_meteo in ["summer-drought"] and magnitude == 0 and duration == 3:
                            scenario_flags.append('--stress-test-meteo %s --stress-test-meteo-magnitude %s --stress-test-meteo-duration %s --soil-compaction soil-compaction --irrigation no-irrigation --stress-test-well-extraction stress' % (stress_test_meteo, magnitude, duration))
                            scenario_flags.append('--stress-test-meteo %s --stress-test-meteo-magnitude %s --stress-test-meteo-duration %s --soil-compaction soil-compaction --irrigation irrigation --stress-test-well-extraction stress' % (stress_test_meteo, magnitude, duration))
                            script_names.append('write_modflow_data_%s-magnitude%s-duration%s_no-irrigation_no-yellow-mustard_soil-compaction_well-extraction-stress' % (stress_test_meteo, magnitude, duration))
                            script_names.append('write_modflow_data_%s-magnitude%s-duration%s_irrigation_no-yellow-mustard_soil-compaction_well-extraction-stress' % (stress_test_meteo, magnitude, duration))
    ...
    for scenario_flag, script_name in zip(scenario_flags, script_names):
        ...
        lines.append("\n")
        lines.append('python write_binary_to_netcdf_transient.py %s\n' % (scenario_flag))
