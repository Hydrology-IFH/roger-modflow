...
def main(model_run):
    ...
    dwp_areas = ["wsg_hausen", "wsg_zartener_becken"]
    ...
    mask_porous = ds_params["mask_porous_aquifer"].values
    ...
    for stress_test_scenario in stress_test_scenarios:
        ...
        for year in years:
            ...
            for i in range(_direct_recharge_year.shape[0]):
                ...
                direct_recharge_day = np.where(mask_porous, direct_recharge_day, 0)
        ...
        if "well-extraction-stress" in stress_test_scenario:
            if "summer-drought" in stress_test_scenario:
                if "magnitude2-duration3" in stress_test_scenario:
                    file = base_path.parent / "input" / "stress_tests_well_extraction" / "summer-drought" / "duration3_magnitude2" / "drinking_water_well_extraction_daily.csv"
                elif "magnitude2-duration0" in stress_test_scenario:
                    file = base_path.parent / "input" / "stress_tests_well_extraction" / "summer-drought" / "duration0_magnitude2" / "drinking_water_well_extraction_daily.csv"
                elif "magnitude0-duration3" in stress_test_scenario:
                    file = base_path.parent / "input" / "stress_tests_well_extraction" / "summer-drought" / "duration3_magnitude0" / "drinking_water_well_extraction_daily.csv"
        else:
            file = base_path.parent / "input" / "drinking_water_well_extraction_daily.csv"
        df_drinking_water_well_extraction_daily = pd.read_csv(file, sep=";", index_col=0, skiprows=1)
        df_drinking_water_well_extraction_daily.index = pd.to_datetime(df_drinking_water_well_extraction_daily.index, format="%Y-%m-%d")
        df_drinking_water_well_extraction_daily = df_drinking_water_well_extraction_daily.loc[date_time]  # select only the time period of interest
        # resample to monthly
        ...
        df_extraction_balance_monthly["extraction_balance"] =  ((df_direct_recharge_monthly["direct_recharge"] + df_indirect_recharge_monthly["indirect_recharge"]) * 0.3) - df_drinking_water_well_extraction_monthly["well_extraction"]
        ...
        df_extraction_balance_annual["extraction_balance"] = ((df_direct_recharge_annual["direct_recharge"] + df_indirect_recharge_annual["indirect_recharge"]) * 0.3) - df_drinking_water_well_extraction_annual["well_extraction"]
        ...
        df_drinking_water_well_extraction_annual["well_extraction"] = df_drinking_water_well_extraction_annual["well_extraction"] / 1e6
        df_drinking_water_well_extraction_annual.plot(kind="bar", ax=ax, color="purple", width=0.8)
        # rotate xticklabels to vertical and show only the year
        ax.set_xticklabels(df_drinking_water_well_extraction_annual.index.year, rotation=0)
        ax.set_xlabel("Jahr")
        ...
        fig.tight_layout()
        fig.savefig(figures_dir / "drinking_water_well_extraction_annual.png", dpi=300)

        # plot long-term average annual drinking water well extraction
        fig, ax = plt.subplots(figsize=(2, 2))
        long_term_average = df_drinking_water_well_extraction_annual["well_extraction"].mean()
        ...
        ax.legend().set_visible(False)
        fig.tight_layout()
        fig.savefig(figures_dir / "drinking_water_well_extraction_annual_long_term_average.png", dpi=300)
        ...
        ax.legend().set_visible(False)
        fig.tight_layout()
        fig.savefig(figures_dir / "recharge_long_term_stacked.png", dpi=300)
        ...
        # reformat xticklabels to show only the year
        ax.set_xticklabels(df_extraction_balance_annual.index.year, rotation=0)
        ax.set_xlabel("Jahr")
        ax.set_ylabel("GW-Entnahmebilanz\n[Mio. m³/Jahr]")
        fig.tight_layout()
