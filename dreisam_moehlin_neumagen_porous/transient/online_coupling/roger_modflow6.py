...
file_config_modflow = base_path.parent / "config_modflow.yml"
with open(file_config_modflow, "r") as file:
...
class ModFlowSimulation:
    def __init__(
        ...
        gwf = flopy.mf6.ModflowGwf(sim, modelname=name, model_nam_file=model_nam_file, save_flows=False, newtonoptions="NEWTON")
...
def main(stress_test_meteo, stress_test_meteo_magnitude, stress_test_meteo_duration, irrigation, yellow_mustard, soil_compaction, grain_corn_only, stress_test_well_extraction,backend, float_type):
    ...
    file_config_roger = base_path / "config_roger.yml"
    with open(file_config_roger, "r") as file:
    ...
    config_roger['OUTPUT_COLLECT'] = ["theta_rz", "theta_ss"]
    config_roger['OUTPUT_RATE'] = ["q_ss", "cpr_ss"]
    if irrigation == "irrigation":
        config_roger['OUTPUT_RATE'].extend(["irrig"])
    ...
    with xr.open_dataset(file, engine="h5netcdf") as ds:
        ...
        if irrigation == "irrigation":
            config_roger['enable_irrigation'] = True
            config_roger['enable_net_irrigation'] = True
        else:
            config_roger['enable_irrigation'] = False
            config_roger['enable_net_irrigation'] = False

    # save the updated config file
    with open(file_config_roger, "w") as file:
        yaml.dump(config_roger, file)

    # initialize the ONED model of RoGeR using BMI
    ...
    modflow_mask = modflow_interface.modflow_basin 
    ...
    for i in range(NDAYS):
        ...
        groundwater_head = aggregate_to_finer_resolution(groundwater_head, 50, 25, method="keep")
        # RoGeR requires depth of groundwater head (in meters)
        groundwater_depth = topography.flatten() - groundwater_head.flatten()
        groundwater_depth_ = topography.flatten() - groundwater_head.flatten()
        groundwater_depth[(groundwater_depth <= soildepth)] = soildepth[(groundwater_depth <= soildepth)] + 0.01 # constrain groundwater depth to soil depth
        ...
        capillary_rise = np.zeros(roger_interface.get_grid_node_count())
        roger_interface.get_value("cpr_ss", capillary_rise)
        capillary_rise = capillary_rise.reshape(config_roger['nx'], config_roger['ny']).astype(np.float64) / 1000  # mm/day to m/day
        ...
        if irrigation == "irrigation":
            irrigation_ = np.zeros(roger_interface.get_grid_node_count())
            roger_interface.get_value("cpr_ss", irrigation_)
            irrigation_ = irrigation_.reshape(config_roger['nx'], config_roger['ny']).astype(np.float64) / 1000  # mm/day to m/day
            ...
            irrigation_ = aggregate_to_coarser_resolution(irrigation_, 25, 50, method="average")
