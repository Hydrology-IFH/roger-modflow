...
path = Path(__file__).parent / "input" / "parameters_50m.nc"
ds_params = xr.open_dataset(path)
topography = ds_params['dgm'].values


# # load topography and domain
# topography = ds_params['elevations'].isel(band=0).values
# elevation_bottom_layer1 = ds_params['elevations'].isel(band=1).values
# topography[topography <= elevation_bottom_layer1] = elevation_bottom_layer1[topography <= elevation_bottom_layer1]
# elevation_bottom_layer2 = ds_params['elevations'].isel(band=2).values
...
# elevation_bottom_layer3 = ds_params['elevations'].isel(band=3).values
...
# elevation_bottom_layer4 = ds_params['elevations'].isel(band=4).values
...
fig, axes = plt.subplots(figsize=(4, 4))
axes.scatter(river_reach['j'].values*modflow_config['dx'], (np.max(river_reach['i'].values) - river_reach['i'].values)*modflow_config['dy'], s=0.001, c='blue')
cb = axes.imshow(topography, extent=grid_extent, cmap='terrain', aspect='equal')
fig.colorbar(cb, ax=axes, label='[m a.s.l.]', shrink=0.5)
axes.grid(zorder=0)
axes.set_xlabel('Distance in x-direction [m]')
...
# hydraulic_conductivities_layer1 = ds_params['kf'].isel(layer=0).values / (60 * 60 * 24)
# hydraulic_conductivities_layer2 = ds_params['kf'].isel(layer=1).values / (60 * 60 * 24)
# hydraulic_conductivities_layer3 = ds_params['kf'].isel(layer=2).values / (60 * 60 * 24)
# hydraulic_conductivities_layer4 = ds_params['kf'].isel(layer=3).values / (60 * 60 * 24)
