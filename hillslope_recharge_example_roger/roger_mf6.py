...
def main():
    ...
    NAME = 'hillslope'
    NLAY = 1
    SOILDEPTH = soildepth.reshape(domain['nrow'], domain['ncol']).mean() / 1000
    ...
    modlfow_interface = ModFlowSimulation(
        NAME,
        base_path,
        ndays=NDAYS,
        nlay=NLAY,
    ...
    for i in range(NDAYS):
        ...
        modlfow_interface.get_groundwater_head(groundwater_head)
        roger_interface.set_value("z_gw", groundwater_head)
        ...
        modlfow_interface.set_recharge(recharge)

        # run MODFLOW for one timestep
        modlfow_interface.step(plot=False)
    
    roger_interface.finalize()
    modlfow_interface.finalize()
    print("RoGeR and MODFLOW finalized")
