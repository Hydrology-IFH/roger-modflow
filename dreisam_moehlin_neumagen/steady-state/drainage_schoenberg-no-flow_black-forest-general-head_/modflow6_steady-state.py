...
class ModFlowSimulation:
    def __init__(
        ...
        index = np.where((mask_boundary_condition == 1) & (mask_boundary_condition_schoenberg == 0) & (mask_boundary_condition_black_forest == 0))
        rows_bc = index[0]
        cols_bc = index[1]
        ...
        index = np.where((mask_boundary_condition_schoenberg == 1) & (mask_boundary_condition_black_forest == 0) & (mask_boundary_condition == 0))
        rows_bc = index[0]
        cols_bc = index[1]
        for ii in range(0, len(rows_bc)):
        ...
        index = np.where((mask_boundary_condition_black_forest == 1) & (mask_boundary_condition == 0) & (mask_boundary_condition_schoenberg == 0))
        rows_bc = index[0]
        cols_bc = index[1]
        for ii in range(0, len(rows_bc)):
