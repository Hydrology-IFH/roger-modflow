...
def main(model_run):
    ...
    elevation_bottom_layers = [elevation_bottom_layer]
    elevation_layers = [topography, elevation_bottom_layer]

    hydraulic_conductivities_layer = ds_params['kf'].isel(layer=2).values
    ...
    ll_levels = [[335, 340, 350, 360, 375, 400, 450, 500],
                 [335, 340, 350, 360, 375, 400, 450, 500],
                 [335, 340, 350, 360, 375, 400, 450, 500],
                 [335, 340, 350, 360, 375, 400, 450, 500]]

    for layer in range(1):
        fig, axes = plt.subplots(figsize=(4, 4))
        plt.imshow(ds_mf['head'].isel(Time=0, layer=layer).values[ylim1:ylim2, xlim1:xlim2], cmap='viridis', aspect='equal')
        ...
        fig, axes = plt.subplots(figsize=(4, 4))
        gw_depth = topography[ylim1:ylim2, xlim1:xlim2] - ds_mf['head'].isel(Time=0, layer=layer).values[ylim1:ylim2, xlim1:xlim2]
        ...
        fig, axes = plt.subplots(figsize=(4, 4))
        y = np.arange(0, ylim2 - ylim1, 1)
        x = np.arange(0, xlim2 - xlim1, 1)
        X, Y = np.meshgrid(x, y)
        Z = ds_mf['head'].isel(Time=0, layer=layer).values[ylim1:ylim2, xlim1:xlim2]
        levels = ll_levels[layer]
        CS = axes.contour(X, Y, Z, levels, colors='black')
        axes.clabel(CS, inline=True, fontsize=8, colors='black')
        plt.imshow(ds_mf['head'].isel(Time=0, layer=layer).values[ylim1:ylim2, xlim1:xlim2], cmap='viridis', aspect='equal', alpha=0.5)
        plt.xlabel('x-direction')
        plt.ylabel('y-direction')
    ...
    for layer in range(1):
        ...
        thickness = elevation_layers[layer][ylim1:ylim2, xlim1:xlim2] - elevation_layers[layer + 1][ylim1:ylim2, xlim1:xlim2]
        fig, axes = plt.subplots(figsize=(4, 4))
        ...
        hydraulic_conductivity = hydraulic_conductivities_layers[layer][ylim1:ylim2, xlim1:xlim2]
        fig, axes = plt.subplots(figsize=(4, 4))
        ...
        hydraulic_conductivity = hydraulic_conductivities_layers[layer][ylim1:ylim2, xlim1:xlim2]
        fig, axes = plt.subplots(figsize=(4, 4))
        bounds = [0.000001, 0.00001, 0.0001, 0.001, 0.01, 0.1, 1, 10]
        norm = mpl.colors.BoundaryNorm(bounds, mpl.colormaps["Oranges"].N)
        plt.imshow(hydraulic_conductivity, extent=grid_extent, cmap='Oranges', aspect='equal', norm=norm)
        cbar = plt.colorbar(label='$k_f$ [m/day]', shrink=0.45)
        cbar.set_ticks(ticks=bounds, labels=[r'$10^{-6}$', r'$10^{-5}$', r'$10^{-4}$', r'$10^{-3}$', r'$10^{-2}$', r'$10^{-1}$', '1', '10'])
        plt.grid(zorder=0)
        plt.xlabel('Distance in x-direction [m]')
        plt.ylabel('Distance in y-direction [m]')
        plt.tight_layout()
        i = layer + 1
        file = base_path_figs / f"_kf_layer{i}_{model_run}_.png"
        fig.savefig(file, dpi=300)
        plt.close("all")
    return
