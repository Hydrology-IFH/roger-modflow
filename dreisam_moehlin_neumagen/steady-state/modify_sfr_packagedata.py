...
gdf_reaches = gpd.read_file(base_path / "input" / "streamflow_routing_sfr_lines.shp")

# load the manning coefficient raster
base_path = Path(__file__).parent
src = rasterio.open(str(base_path / "input" / "streamflow_routing" / "manning.tif"))
manning = src.read(1)

# load the streambed conductivity raster
base_path = Path(__file__).parent
src = rasterio.open(str(base_path / "input" / "streamflow_routing" / "streambed_conductivity.tif"))
streambed_conductivity = src.read(1)

# load the fraction of channelisation raster
base_path = Path(__file__).parent
src = rasterio.open(str(base_path / "input" / "streamflow_routing" / "fraction_of_channelisation.tif"))
fraction_of_channelisation = src.read(1)

# load the streambed structure raster
base_path = Path(__file__).parent
src = rasterio.open(str(base_path / "input" / "streamflow_routing" / "streambed_structure.tif"))
