...
def main(offset, plot):
    ...
    mask_boundary_condition_porous_aquifer = np.where((boundary == 1) & (topography <= 380), 1, np.nan)
    ...
    # df_river = pd.read_csv(base_path / "river_xy.csv", sep=";")
    # river_y = df_river["y"].values
    # river_x = df_river["x"].values
    # df_river["elevation"] = topography[ylim1 + river_y, xlim1 + river_x].flatten()
    # df_river["elevation_interp"] = np.linspace(np.min(topography[ylim1 + river_y, xlim1 + river_x].flatten()), np.max(topography[ylim1 + river_y, xlim1 + river_x].flatten()), len(river_y)) - 1
    # df_river["elevation_interp"][4:-4] = moving_average(df_river["elevation"].values, 9).astype(np.float32) - 2
    # df_river["depth"] = df_river["elevation"] - df_river["elevation_interp"]
    # df_river["elevation_interp"] = np.where(df_river["depth"] > 1.5, df_river["elevation_interp"] + (df_river["depth"] - 1.5), df_river["elevation_interp"])
    # df_river["depth"] = df_river["elevation"] - df_river["elevation_interp"]
    # df_river.to_csv(base_path / "river_xy.csv", index=False, sep=";")

    # river_y = np.where(topography[ylim1:ylim2, xlim1:xlim2]==np.min(topography[ylim1:ylim2, xlim1:xlim2], axis=0))[0]
    # river_x = np.where(topography[ylim1:ylim2, xlim1:xlim2]==np.min(topography[ylim1:ylim2, xlim1:xlim2], axis=0))[1]
    # river_elevation = topography[river_y, river_x]
    # pd.DataFrame({"x": river_x, "y": river_y}).to_csv(base_path / "river_xy.csv", index=False, sep=";")
