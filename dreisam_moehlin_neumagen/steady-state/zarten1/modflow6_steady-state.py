...
class ModFlowSimulation:
    def __init__(
        ...
        mask = (ds_params['mask_upper_dreisam'].values == 1) & (topography < 380)
        ...
        hydraulic_conductivities_layer1[:, :] = 0.47
        hydraulic_conductivities_layer2[:, :] = 0.42
        hydraulic_conductivities_layer3[:, :] = 0.37
        hydraulic_conductivities_layer4[:, :] = 0.37
        ...
        for ii in range(0, len(rows_bc)):
            constant_head = ds_bc['constant_head_porous_aquifer'].values[rows_bc[ii], cols_bc[ii]] - 7
