...
for i, hydraulic_conductivities_layer in enumerate(hydraulic_conductivities_layers):
    i = i + 1
    fig, axes = plt.subplots(figsize=(4, 4))
    bounds = np.unique(hydraulic_conductivities_layer[mask]/86400).tolist()
    ...
    cbar.set_ticks(ticks=bounds, labels=["{:.1e}".format(val) for val in np.unique(hydraulic_conductivities_layer[mask]/86400).tolist()])
    plt.grid(zorder=0)
    plt.xlabel('Distance in x-direction [km]')
    plt.ylabel('Distance in y-direction [km]')
    plt.tight_layout()
