...
rnos = []
labels = []
# get reach number of observation points
for x, y in zip(df_observation_points["cell_x"].values, df_observation_points["cell_y"].values):
    cond = (df_reaches["j"].values == x) & (df_reaches["i"].values == y)
    cond1 = (df_observation_points["cell_x"].values == x) & (df_observation_points["cell_y"].values == y)
    if cond.any():
        rno = df_reaches.loc[cond, "rno"].values[0]
        label = df_observation_points.loc[cond1, "Label"].values[0]
        rnos.append(rno)
        labels.append(label)
