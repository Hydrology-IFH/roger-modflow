...
class ModFlowSimulation:
    def __init__(
        ...
        # hydraulic_conductivities_layer1[:, :] = 1.8
        # hydraulic_conductivities_layer2[:, :] = 1.8
        # hydraulic_conductivities_layer3[:, :] = 1.8
        # hydraulic_conductivities_layer4[:, :] = 1.8
        ...
        for ii in range(0, len(rows_bc)):
            if cols_bc[ii] == xlim1:
                ...
                chd_rec.append(((2, rows_bc[ii], cols_bc[ii]), constant_head))
            elif cols_bc[ii] == xlim2-1:
                constant_head = ds_bc['constant_head_porous_aquifer'].values[rows_bc[ii], cols_bc[ii]]
                chd_rec.append(((2, rows_bc[ii], cols_bc[ii]), constant_head))
