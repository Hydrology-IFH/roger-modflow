...
class ModFlowSimulation:
    def __init__(
        ...
        mask = np.where(mask_boundary_condition_schoenberg == 1, True, mask)
        ...
        mask = np.where(mask_boundary_condition_black_forest == 1, True, mask)
        ...
        for ii in range(0, len(rows_bc)):
            if mask[rows_bc[ii], cols_bc[ii]]:
                constant_head = ds_bc['constant_head_schoenberg'].values[rows_bc[ii], cols_bc[ii]] - fudge_parameters['offset'].values[model_run]
                width = 50
                if (constant_head <= topography[rows_bc[ii], cols_bc[ii]]) and (constant_head > elevation_bottom_layer1[rows_bc[ii], cols_bc[ii]]):
                    layer = 0
        ...
        wells_q = [5727, 5822, 3494, 4315, 4525, 2899, 6401, 7024, 3160, 1117, 920, 1340]
        # location of the wells
        wells_y = [266, 268, 271, 272, 280, 259, 210, 212, 217, 225, 232, 228]
        wells_x = [66, 64, 63, 59, 56, 88, 464, 464, 465, 465, 477, 459]
        wel_rec = []
