...
class ModFlowSimulation:
    def __init__(
        ...
        mask = (topography > 270) & (topography < 380)
        mask[:ylim1, :] = False
        mask[ylim2:, :] = False
        mask[:, :xlim1] = False
        mask[:, xlim2:] = False
        mask[ylim1+70:, :xlim1+20] = False
        mask[:ylim1+12, :xlim1+10] = False
        mask[:ylim1+6, xlim1+10:xlim1+30] = False        
        ...
        hydraulic_conductivities_layer1 = ds_params['kf'].isel(layer=0).values * 1
        ...
        mask1 = (hydraulic_conductivities_layer2 < 1) & mask
        mask2 = (topography >= 320) & (hydraulic_conductivities_layer2 >= 1) & mask
        mask3 = (topography < 320) & (hydraulic_conductivities_layer2 >= 1) & mask
        hydraulic_conductivities_layer2[mask1] = hydraulic_conductivities_layer2[mask1] * 1
        hydraulic_conductivities_layer3[mask1] = hydraulic_conductivities_layer2[mask1] * 1
        hydraulic_conductivities_layer4[mask1] = hydraulic_conductivities_layer2[mask1] * 1
        hydraulic_conductivities_layer2[mask2] = hydraulic_conductivities_layer2[mask2] * 1
        hydraulic_conductivities_layer3[mask2] = hydraulic_conductivities_layer2[mask2] * 1
        hydraulic_conductivities_layer4[mask2] = hydraulic_conductivities_layer2[mask2] * 1
        hydraulic_conductivities_layer2[mask3] = hydraulic_conductivities_layer2[mask3] * 1
        hydraulic_conductivities_layer3[mask3] = hydraulic_conductivities_layer2[mask3] * 1
        hydraulic_conductivities_layer4[mask3] = hydraulic_conductivities_layer2[mask3] * 1
        ...
        for ii in range(0, len(rows_bc)):
            constant_head = topography[rows_bc[ii], cols_bc[ii]] - 45
            chd_rec.append(((1, rows_bc[ii], cols_bc[ii]), constant_head))
            print(((rows_bc[ii], cols_bc[ii]), constant_head))
        ...
        cols_bc = index[1]
        for ii in range(0, len(rows_bc)):
            if mask[rows_bc[ii], cols_bc[ii]] and hydraulic_conductivities_layer2[rows_bc[ii], cols_bc[ii]] >= 1 and cols_bc[ii] == xlim1:
                constant_head = ds_bc['constant_head_porous_aquifer'].values[rows_bc[ii], cols_bc[ii]] - 10
                width = 50
                _c = 0.01
                if (constant_head <= topography[rows_bc[ii], cols_bc[ii]]) and (constant_head > elevation_bottom_layer1[rows_bc[ii], cols_bc[ii]]):
                    layer = 0
                    thickness = constant_head - elevation_bottom_layer1[rows_bc[ii], cols_bc[ii]]
                    conductance = hydraulic_conductivities_layers[layer][rows_bc[ii], cols_bc[ii]] * thickness * width * _c
                    ...
                    layer = 1
                    thickness = constant_head - elevation_bottom_layer2[rows_bc[ii], cols_bc[ii]]
                    conductance = hydraulic_conductivities_layers[layer][rows_bc[ii], cols_bc[ii]] * thickness * width * _c
                    ...
                    thickness = constant_head - elevation_bottom_layer3[rows_bc[ii], cols_bc[ii]]
                    conductance = hydraulic_conductivities_layers[layer][rows_bc[ii], cols_bc[ii]] * thickness * width * _c
            # elif mask[rows_bc[ii], cols_bc[ii]] and hydraulic_conductivities_layer2[rows_bc[ii], cols_bc[ii]] >= 1:
            ...
            #         conductance = hydraulic_conductivities_layers[layer][rows_bc[ii], cols_bc[ii]] * thickness * width * _c
                ...
                #     layer = 3
                #     thickness = constant_head - elevation_bottom_layer4[rows_bc[ii], cols_bc[ii]]
                #     conductance = hydraulic_conductivities_layers[layer][rows_bc[ii], cols_bc[ii]] * thickness * width 
                #     ghb_rec.append([layer, rows_bc[ii], cols_bc[ii], constant_head, conductance])

                # if mask[rows_bc[ii], cols_bc[ii]] and hydraulic_conductivities_layer2[rows_bc[ii], cols_bc[ii]] >= 1 and (constant_head > elevation_bottom_layer1[rows_bc[ii], cols_bc[ii]]):
                #     layer = 1
                #     thickness = elevation_bottom_layer1[rows_bc[ii], cols_bc[ii]] - elevation_bottom_layer2[rows_bc[ii], cols_bc[ii]]
                #     conductance = hydraulic_conductivities_layers[layer][rows_bc[ii], cols_bc[ii]] * thickness * width 
                #     ghb_rec.append([layer, rows_bc[ii], cols_bc[ii], elevation_bottom_layer1[rows_bc[ii], cols_bc[ii]], conductance])
                # if mask[rows_bc[ii], cols_bc[ii]] and hydraulic_conductivities_layer2[rows_bc[ii], cols_bc[ii]] >= 1 and (constant_head > elevation_bottom_layer1[rows_bc[ii], cols_bc[ii]]):                    
                #     layer = 2
                #     thickness = elevation_bottom_layer2[rows_bc[ii], cols_bc[ii]] - elevation_bottom_layer3[rows_bc[ii], cols_bc[ii]]
                #     conductance = hydraulic_conductivities_layers[layer][rows_bc[ii], cols_bc[ii]] * thickness * width 
                #     ghb_rec.append([layer, rows_bc[ii], cols_bc[ii], elevation_bottom_layer2[rows_bc[ii], cols_bc[ii]], conductance])
                # if mask[rows_bc[ii], cols_bc[ii]] and hydraulic_conductivities_layer2[rows_bc[ii], cols_bc[ii]] >= 1 and (constant_head > elevation_bottom_layer3[rows_bc[ii], cols_bc[ii]]):
                #     layer = 3
                #     thickness = elevation_bottom_layer3[rows_bc[ii], cols_bc[ii]] - elevation_bottom_layer4[rows_bc[ii], cols_bc[ii]]
                #     conductance = hydraulic_conductivities_layers[layer][rows_bc[ii], cols_bc[ii]] * thickness * width 
                #     ghb_rec.append([layer, rows_bc[ii], cols_bc[ii], elevation_bottom_layer3[rows_bc[ii], cols_bc[ii]], conductance])
        ...
        rcha = flopy.mf6.ModflowGwfrcha(gwf, recharge=recharge * 0.9, fixed_cell=True)

        # Create the well package (Neumann boundary condition i.e. second type)
        # pumping rate in m3/day
        wells_q = [6401, 7024, 3160, 1117, 920, 1340]
        # location of the wells
        wells_y = [210, 212, 217, 225, 232, 228]
        wells_x = [464, 464, 465, 465, 477, 459]
        wel_rec = []
