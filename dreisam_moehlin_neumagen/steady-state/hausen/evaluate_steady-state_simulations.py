...
cols = observed_groundwater_heads.iloc[:, -2].values  # column IDs of the observation wells
...
for model_run in range(0, 5000):
    ...
    sim = topography[rows, cols].flatten() - groundwater_heads[1, rows, cols].flatten()
    no_nas = np.isfinite(sim)
    ...
    # calculate spearman correlation
    df_params_metrics.loc[model_run, "r"] = sp.stats.spearmanr(sim[no_nas], obs[no_nas])[0]
