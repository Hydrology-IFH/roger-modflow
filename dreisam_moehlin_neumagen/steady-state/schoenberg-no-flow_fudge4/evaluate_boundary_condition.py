...
def main(model_run, plot):
    try:
        ...
        if plot:
            ...
            plt.tight_layout()
            file = Path(__file__).parent / "figures" / "gradients_grid.png"
        ...
        if plot:
            ...
            fig.tight_layout()
            file = Path(__file__).parent / "figures" / "evaluation_of_constant_head.png"
            fig.savefig(file, dpi=300)
            plt.close("all")
            ...
            axes.set_ylabel("[m]")
            fig.tight_layout()
            file = Path(__file__).parent / "figures" / "gradient_constant_head.png"
            fig.savefig(file, dpi=300)
            plt.close("all")
    except:
        pass
