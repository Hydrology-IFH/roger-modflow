...
class ModFlowSimulation:
    def __init__(
        ...
        hydraulic_conductivity = ds_params['kf'].isel(layer=3).values
        ...
        hydraulic_conductivities_layer = ds_params['kf'].isel(layer=3).values * 0.9
        ...
        for i in range(len(wells_x)):
            wel_rec.append((0, wells_y[i], wells_x[i], -wells_q[i]))  # extraction from layer 3
