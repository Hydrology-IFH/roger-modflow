...
fig, axes = plt.subplots(figsize=(4, 4))
axes.scatter(sim, obs, marker='.', s=5, c='black')
axes.plot(axes.get_xlim(), axes.get_ylim(), ls="--", c=".3", zorder=1, alpha=0.5)
axes.set_xlabel('Simulated Groundwater head [m a.s.l.]')
axes.set_ylabel('Observed Groundwater head [m a.s.l.]')
# axes.text(axes.get_xlim()[0] + 0.1, axes.get_ylim()[1] - 0.1, f"ME: {df_params_metrics.loc[model_run, 'ME']:.2f} m")
fig.tight_layout()
file = Path(__file__).parent / "figures" / "steady-state" / f"scatter_obs_sim{model_run}.png"
fig.savefig(file, dpi=300)
plt.close(fig)
...
fig.tight_layout()
file = Path(__file__).parent / "figures" / "steady-state" / f"scatter_diff_obs_sim{model_run}.png"
fig.savefig(file, dpi=300)
plt.close(fig)
