...
def main(backend, float_type):
    ...
    roger_config['OUTPUT_RATE'] = ["q_hof", "q_ss", "cpr_ss"]
    ...
    well_mask = np.empty((modflow_config['nz'], modflow_config['nx'], modflow_config['ny']))
    well_mask[:, :, :] = False
    well_mask[-1, 10, 82] = True
    well_mask[-1, 40, 92] = True
    well_mask[-1, 85, 62] = True
    well_mask = well_mask.astype(bool)

    # initialize the SVAT model of RoGeR using BMI
    model = SVATSetup(base_path)
    roger_interface = BmiRoger(model=model, enable_groundwater_boundary=True)
    ...
    # set parameters for capillary rise
    evt_surface = np.zeros(modflow_config['nx'] * modflow_config['ny'])
    evt_surface = evt_surface.reshape(modflow_config['nx'], modflow_config['ny']).astype(np.float64)
    evt_surface[:, :] = topography - soildepth.reshape(modflow_config['nx'], modflow_config['ny']) 
    modflow_interface.set_evt_surface(evt_surface)

    evt_depth = np.zeros(modflow_config['nx'] * modflow_config['ny'])
    evt_depth = evt_depth.reshape(modflow_config['nx'], modflow_config['ny']).astype(np.float64)
    evt_depth[:, :] = 1
    modflow_interface.set_evt_depth(evt_depth)
    ...
    for i in range(NDAYS):
        ...
        well_rate1 = -RNG.uniform(10, 30, size=modflow_config['nx'] * modflow_config['ny']).reshape(modflow_config['nx'], modflow_config['ny']).astype(np.float64)
