...
x_ = np.random.uniform(xcoords[min_col], xcoords[max_col], 300)
y_ = np.random.uniform(ycoords[min_row], ycoords[max_row], 300)
...
with h5netcdf.File(file, "a", decode_vlen_strings=False) as f:
    ...
    v = f.create_variable(
        "initial_heads", ("y", "x"), np.float64, compression="gzip", compression_opts=1
    )
    v[:, :] = uk_heads[:, :]
