...
def main():
    ...
    model = SVATSetup(base_path, enable_groundwater_boundary=True)
    ...
    evt_surface = np.zeros(domain['nrow'] * domain['ncol'])
    evt_surface = evt_surface.reshape(domain['nrow'], domain['ncol']).astype(np.float64)
    evt_surface[:, :] = topography - soildepth.reshape(domain['nrow'], domain['ncol']) 
    modflow_interface.set_evt_surface(evt_surface)

    evt_depth = np.zeros(domain['nrow'] * domain['ncol'])
    evt_depth = evt_depth.reshape(domain['nrow'], domain['ncol']).astype(np.float64)
    evt_depth[:, :] = 1
