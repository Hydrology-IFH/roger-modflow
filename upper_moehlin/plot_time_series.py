...
axes.set_xlim(df_heads.index[0], df_heads.index[-1])
fig.tight_layout()
path = base_path_figs / "observed groundwater_head_PH_037_.png"
fig.savefig(path, dpi=300)
...
output_file = base_path / "output" / model_type / "svat" / "modflow_output.nc"
ds_mf = xr.open_dataset(output_file, engine="h5netcdf")
ndays = ds_mf.sizes['time']

output_file = base_path / "output" / model_type / "svat" / "roger_output.nc"
...
#     fig, axes = plt.subplots(figsize=(5, 2))
#     axes.plot(dates, vals_avg, ls='--', label='Mittelwert', lw=1.5, color='black')
#     axes.plot(dates, vals_p50, ls='-', label='Median', lw=2, color='black')
#     axes.plot(dates, vals_min, ls='-.', label='Min/Max', alpha=0.5, color='black', lw=1.0)
#     axes.plot(dates, vals_max, ls='-.', alpha=0.5, color='black', lw=1.0)
#     axes.set_xlabel('Zeit [Jahr/Monat]')
#     axes.set_ylabel('Grundwasserspiegel \n[m ü.NN]')
#     axes.set_xlim(dates[0], dates[-1])
#     # axes.legend(frameon=False, fontsize=9)
#     axes.tick_params(axis="x", rotation=33)
#     fig.tight_layout()
#     path = base_path_figs / f"heads_time_series_layer{layer}_ger.png"
#     fig.savefig(path, dpi=300)
...
# # axes.plot(dates, vals_p95, ls='-.', alpha=0.5, color='black', lw=1.0)
# axes.set_xlabel('Zeit [Jahr/Monat]')
# axes.set_ylabel('Grundwasserneubildung \n[mm/Tag]')
# axes.set_xlim(dates[0], dates[-1])
# axes.set_ylim(0, 150)
# axes.tick_params(axis="x", rotation=33)
# axes.legend(frameon=False, fontsize=9, ncol=3)
# fig.tight_layout()
# path = base_path_figs / "recharge_time_series_ger.png"
# fig.savefig(path, dpi=300)
...
vals = ds_mf['head'].isel(layer=3).values
vals1 = vals[:, 15, 92]
vals2 = vals[:, 160, 60]
fig, axes = plt.subplots(figsize=(5, 2))
axes.plot(dates, vals1, ls='-', lw=1.5, color='magenta')
axes.plot(dates, vals2, ls='-', lw=1, color='grey')
axes.set_xlabel('Zeit [Jahr/Monat]')
axes.set_ylabel('Grundwasserspiegel \n[m ü.NN]')
axes.set_xlim(dates[0], dates[-1])
# axes.legend(frameon=False, fontsize=9)
axes.tick_params(axis="x", rotation=33)
fig.tight_layout()
path = base_path_figs / "heads_time_series_layer3_sites_ger.png"
fig.savefig(path, dpi=300)

vals = ds_roger['q_ss'].values[:, :, 1:]
vals[vals < 0] = np.nan
vals1 = vals[15*2, 92*2, :]
vals2 = vals[160*2, 60*2, :]
fig, axes = plt.subplots(figsize=(5, 2))
axes.plot(dates, vals1, ls='-', lw=1.5, color='magenta')
axes.plot(dates, vals2, ls='-', lw=1, color='grey')
axes.set_xlabel('Time [Jahr/Monat]')
axes.set_ylabel('Grundwasserneubildung \n[mm/Tag]')
axes.set_xlim(dates[0], dates[-1])
axes.set_ylim(0, 40)
axes.tick_params(axis="x", rotation=33)
# axes.legend(frameon=False, fontsize=9, ncol=3)
fig.tight_layout()
path = base_path_figs / "recharge_time_series_sites_ger.png"
fig.savefig(path, dpi=300)
