...
def main():
    ...
    for i, nxny in enumerate(rowscols):
        ...
        df_time.iloc[i, 2] = elapsed_time_seconds
        print("MODFLOW (steady-state) finalized simulation for {} cells in {} seconds".format(nx * ny, elapsed_time_seconds))
        df_time.to_csv(base_path / "benchmark_times_modflow_steady-state.csv", index=False, sep=";")
