...
@click.option("-b", "--backend", type=click.Choice(["numpy", "jax"]), default="jax")
...
def main(backend):
    ...
    for i, nxny in enumerate(rowscols):
        ...
        ny = nxny[1]
        modflow_config = {
            'dx': int(roger_config['dx']*2),
            'dy': int(roger_config['dy']*2),
            'nx': int(nx/2),
            'ny': int(ny/2),
        }
        ...
        soildepth = soildepth.reshape(roger_config['nx'], roger_config['ny'])
        soildepth = aggregate_to_coarser_resolution(soildepth, roger_config['dx'], modflow_config['dx'], method="average")
        ...
        for t in range(NDAYS):
            ...
            groundwater_head = np.zeros(int(modflow_config['nx'] * modflow_config['ny']))
        ...
        df_time.to_csv(base_path / f"benchmark_times_roger_{backend}_aggregate_modflow_transient.csv", index=False, sep=";")
